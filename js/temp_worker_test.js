...
self.onmessage = async function(e) {
    ...
    for (let i = 0; i < layerList.length; i++) {
        ...
        let buffer = new Float32Array(numPaths * 11);
        ...
        for (let j = 0; j < numPaths; j++) {
            ...
            if (pType === 1 && fVal > 0) {
                ...
                if (eDist > 0 && s > 0) {
                    ...
                    if (flowRate > 15) {
                        diagnosticWarnings.push({ type: 'high_flow', x: p.x1, y: p.y1, msg: 'High volumetric flow: ' + flowRate.toFixed(1) });
                        healthScore -= 1;
                    }
                }
            }
            ...
            buffer[j * 11 + 4] = p.y2;
            buffer[j * 11 + 5] = fVal;
            buffer[j * 11 + 6] = flowRate;
            buffer[j * 11 + 7] = p.lineIndex || 0;
            buffer[j * 11 + 8] = p.dt || 0;
            buffer[j * 11 + 9] = v_real;
            buffer[j * 11 + 10] = t_segment;
        }
        layer.paths = buffer;
